...
public class MyPageController {
	...
	public String toMyPgae() {
		return "/myPage/myPageMain"; 
	}
	//내가 쓴 글 불러오기 + 페이징
	@RequestMapping("selectMyPost")
	public String selectMyPage(@RequestParam(defaultValue = "1", name = "cpage") int currentPage, Model model) throws Exception {
		// 테스트용 id 세션 생성
...
//		model.addAttribute("myPosts",myPosts);
	...
	public String getMyPostsCount(Model model)throws Exception {
		System.out.println("내가 쓴 글 총 개수 컨트롤러 도착");
		// 테스트용 id 세션 생성
		String userId = "tester123";
		session.setAttribute("loggedID", userId);
		String loggedID = (String) session.getAttribute("loggedID");
